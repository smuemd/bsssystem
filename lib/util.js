...
export function themeGet (path, fallback) {
  return function (attrs = {}, theme = {}) {
    const res = get(attrs.theme || theme, path)
...
export function createStyleVariantFn ({
  ...
  return function styleVariant (attrs = {}, opts) {
    const scale = get(attrs.theme || opts, [ key ])
    return scale
      ? $valueFrom(scale)(attrs[prop])
      : undefined
    // old: return get(attrs.theme || opts, [key, attrs[prop]]) || undefined
  }
}
